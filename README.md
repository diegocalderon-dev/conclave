...
In non-interactive contexts, `--task` is required. In an interactive terminal, `conclave` and `conclave run` will prompt for it when omitted. Interactive prompts accept multiple lines; press `Shift+Enter` to add a new line, `Enter` to submit the task, and `Ctrl+C` to cancel.
